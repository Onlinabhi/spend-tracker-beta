...
function renderMore(){
  ...
  ['personalEnabled','wealthPersonalEnabled'].forEach(id=>{const e=document.getElementById(id);if(e)e.checked=p.enabled!==false});
  ['personalFunny','wealthPersonalFunny'].forEach(id=>{const e=document.getElementById(id);if(e)e.checked=p.funny!==false});
  ['personalFacts','wealthPersonalFacts'].forEach(id=>{const e=document.getElementById(id);if(e)e.checked=p.facts!==false});
  const cats=state.money.cats.map(c=>`<div class="manage-row"><b>${esc(c.icon+' '+c.name)}</b><span></span><button class="mini" onclick="openCategory('${c.id}')">Edit</button><button class="mini danger" onclick="deleteCategory('${c.id}')">Delete</button></div>`).join('')||'<div class="empty">No categories.</div>';
  const methods=state.money.methods.map(m=>`<div class="manage-row"><b>💳 ${esc(m.name)}</b><span>${esc(m.type)}</span><button class="mini" onclick="openMethod('${m.id}')">Edit</button><button class="mini danger" onclick="deleteMethod('${m.id}')">Delete</button></div>`).join('')||'<div class="empty">No payment methods.</div>';
  const income=state.money.incomeAccounts.map(a=>`<div class="manage-row"><b>🏦 ${esc(a.name)}</b><span></span><button class="mini" onclick="openIncomeAccount('${a.id}')">Edit</button><button class="mini danger" onclick="deleteIncomeAccount('${a.id}')">Delete</button></div>`).join('')||'<div class="empty">No income accounts.</div>';
  [['mCatsManage',cats],['wealthCatsManage',cats],['mMethodsManage',methods],['wealthMethodsManage',methods],['mIncomeManage',income],['wealthIncomeManage',income]].forEach(([id,html])=>{const e=document.getElementById(id);if(e)e.innerHTML=html});
}
